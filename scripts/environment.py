...
class energyEnv(gymnasium.Env):
    def __init__(self, data, battery_capacity, energy_idle, energy_frame, delta_time):
    ...
    def reset(self):
        ...
        return self.battery_level, self.irrad, 1.0, self.e_idle, 0
    ...
    def step(self, action):
        ...
        return self.battery_level, self.irrad, 1.0, self.e_idle, reward
    ...
    def update_battery_level(self, value):
        ...
        if((self.battery_level + normalized_value) > self.battery_capacity):
...
def main():
    ...
    env = energyEnv(df_numeric, 10000, 0, 150, 15)
    
    for i in range(1000):
        battery, irradiation, idle, p, partial_reward = env.step(random.randint(0, 1))
        reward += partial_reward
        print(f"battery: {battery}, irradiation: {irradiation}, idle: {idle}, p: {p}, reward: {reward}")
