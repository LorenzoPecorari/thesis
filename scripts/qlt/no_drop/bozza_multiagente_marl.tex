...
\begin{itemize}
    ...
    \item $b_t^i \in [0.0, 1.0]$ percentuale di batteria residua di $i$ in $t$
    \item $\tau_t^i \in [0.0, 1.0]$, percentuale di timestep completati da $i$ al tempo $t$
\end{itemize}
...
\begin{itemize}
    ...
    \item $P_{PV max}$, potenza massima generata dal pannello solare
    ...
    \item $E_{PV, t} = \int{G(t)\cdot A_{PV} \cdot \eta_{PV} \space dt} \approx {G(t)\cdot A_{PV} \cdot \eta_{PV} \cdot \Delta t $, energia in J proveniente dal pannello solare al tempo t
\end{itemize}

\newpage
\section{Agente del nodo}
Ogni nodo è caratterizzato da un agente che tiene conto di una propria Q table con $\Pi_{i \in I}|S^i \times A^i|$ elementi. \\
...
In maniera simile allo spazio degli stati, è possibile definire lo spazio delle azioni caratteroizzato dagli elementi $a_t^i = (f_t^i, x_t^i, g_t^i, h_t^i)$, dove:
\begin{itemize}
...
Date le due dimensioni degli spazi degli stati e delle azioni, la dimensione della Q table risulta essere: $|Q_{table}| = \Pi_{i \in I}|S^i \times A^i| = |S^i| \cdot |A^i| = \Pi_{i \in I}(121 \cdot 3(fps_{max}^i+1)^2\cdot(n+1)) = \Pi_{i \in I} (363(fps_{max}^i+1)^2(n+1)) = (363(fps_{max}^i+1)^2(n+1))^n$\\

Supponendo che il sistema abbia $n = 2$ nodi ed entrambi abbiano $fps_{max} = 30$, la dimensione della tabella sarebbe di $1046529^2 = 1,095222948×10¹²$ elementi. Al fine di ridurre tale valore, si potrebbe adottare una riduzione della dimensione di $f_t^i$ e $h_t^i$, in cui si considerano solo 5 possibili valori che permettono di variare di $fps_{max}^i / 5$ di volta in volta. Adottando tale assunzione, si ottiene che $|Q_{table}| = (363(fps_{max}^i+1)^2(n+1))^n \rightarrow (363(5)^2(2+1))^2 = 27225^2 =741200625$  elementi, una riduzione di 1477.6336 volte. 

\subsection{Aggiornamento della tabella}
...
\begin{algorithm}
    ...
    \ElsIf{($x_t^i =1$ \land $(((f_t^i \cdot E_{F, loc} \cdot \Delta t) + (h_t^i \cdot E_{F, recv} \cdot \Delta t))) < (E_{B,t}^i + E_{PV, t}^i))$)}
