...
'''
    ...
    it bases its functioning over MARL independent-learners, each agent observes
    all the local state of others plus its own. Reward function relies on JIT
    compilation for accelerating the training in terms of time in real hardware.
    Action space is represented by vectors of 4 variables expressing local rate,
    offloading mode, target node and offloading rate. Conditions for offloading,
    if satisfied, allows to extend the local feedback and incentive the cooperation.
'''
...
class CustomEnvironment(ParallelEnv):
    ...
    def step(self, actions):
        ...
        # observations as dictionary of lists
        observations = {
            a: (
                np.array([val for sublist in self.states for val in sublist], dtype=np.float32)
            )
