...
class SB3_MAS_Train_Parallelized_Processes:
    ...
    # print(rewards)
        ...
        plt.tight_layout()
        plt.savefig(f"rewards_plot_{self.num_episodes - 1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_processes.pdf")
        plt.close()
    ...
    def plot_battery_levels(self, levels):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"avg_battery_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_processes.pdf")
        plt.close()
    ...
    def plot_battery_levels_individual(self, levels):
        ...
        for i in range(0, self.env._num_agents):
            ...
            plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
            plt.tight_layout()
            plt.savefig(f"avg_battery_plot_{self.battery_capacities[i]}Wh_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_processes.pdf")
            plt.close()
    ...
    def plot_backlogs(self, backlogs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"backlog_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_processes.pdf")
        plt.close()
    ...
    def plot_battery_daily(self, data):    
        for elem in range(0, self.env._num_agents):
            ...
            plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
            plt.tight_layout()
            plt.savefig(f"battery_{int(self.env.battery_capacities[elem] / 3600)}Wh_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_processes.pdf")
            plt.close()
    ...
    def plot_backlog_daily(self, data):
        ...
        for elem in range(0, self.env._num_agents):
            ...
            plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
            plt.tight_layout()
            plt.savefig(f"backlog_{int(self.env.battery_capacities[elem] / 3600)}Wh_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.num_agents}agents_processes.pdf")
            plt.close()        
    ...
    def plot_framerate(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"framerate_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_processes.pdf")
        plt.close()
    ...
    def plot_local_framerate(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"local_framerate_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_processes.pdf")
        plt.close()
    ...
    def plot_offloading_framerate(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"offloading_framerate_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_processes.pdf")
        plt.close()
    ...
    def plot_offloading_matchings(self, fs):
        ...
        plt.legend(bbox_to_anchor=(0.5, -0.2), loc='upper center', ncol=3)
        plt.tight_layout()
        plt.savefig(f"offloading_matchings_plot_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.w}_{self.env._num_agents}agents_processes.pdf")
        plt.close()

    def save_battery_csv(self, battery_daily):
        
        for agent_id in range(self.num_agents):
            filename = f"./csvs/parallelized_battery_{self.battery_capacities[agent_id]}_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_processes.csv"
    ...
    def save_backlog_csv(self, backlog_daily):
        ...
        for agent_id in range(self.num_agents):
            filename = f"./csvs/parallelized_backlog_{self.battery_capacities[agent_id]}_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_processes.csv"
    ...
    def save_rewards_csv(self, rewards):
        ...
        for agent_id in range(self.num_agents):
            filename = f"./csvs/parallelized_rewards_agent_{self.battery_capacities[agent_id]}_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_processes.csv"
    ...
    def save_time_csv(self, times):
        os.makedirs('./csvs', exist_ok=True)
        filename = f"./csvs/parallelized_time_{self.num_episodes-1}_{self.env.episode}_{self.proc_interval}_{self.num_agents}agents_processes.csv"
