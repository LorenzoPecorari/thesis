...
def conf_comparison(episodes, day, interval, num_agents, mode, mode1, mode2):
    ...
    plt.plot(range(window - 1, len(mode2_vector_single)), np.convolve(mode2_vector_single, np.ones(window)/window, mode='valid'), label = f"{mode1}", alpha = 1.0)
    plt.plot(range(window - 1, len(mode1_vector_single)), np.convolve(mode1_vector_single, np.ones(window)/window, mode='valid'), label = f"{mode2}", alpha = 1.0)
...
conf_comparison(1000, 355, 60, 10, "cuda", "aggregated_states", "reduced_states")