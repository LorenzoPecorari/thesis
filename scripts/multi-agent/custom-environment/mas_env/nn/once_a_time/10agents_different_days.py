...
def compute_avg_rewards(episodes, interval, num_agents):
    rewards = [[0 for i in range(episodes)] for i in range(len(days))]
...
def compute_avg_matchings(episodes, interval, num_agents):
    rewards = [[0 for i in range(episodes+1)] for i in range(len(days))]

    for i in range(len(days)):    
        for agent in range(num_agents):
            with open(f'./csv_different_days/{days[i]}/matchings_{battery_capacities[agent]}_{episodes}_{days[i]}_{interval}_{num_agents}agents_cuda.csv') as file:
                csvFile = csv.reader(file)
                cnt = 0
    ...
    window = 10
    plt.suptitle("Average matchings")
    plt.title(f"Episodes: {episodes}, Interval: {interval}, num_agents: {num_agents}, Mode: cuda")
    ...
    plt.savefig(f"./comparisons/diff_days_average_matchings_episodes_{episodes}_{interval}_{num_agents}agents_cuda.pdf")
    plt.close()
...
compute_avg_rewards(1000, 60, 10)
compute_avg_framerates(1000, 60, 10)
compute_avg_backlog(1000, 60, 10)
compute_avg_battery(1000, 60, 10)
compute_avg_matchings(1000, 60, 10)
