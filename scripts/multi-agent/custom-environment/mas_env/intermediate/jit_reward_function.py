...
def jit_calculate_reward(
    ...
    if(remaining_framerate > 0 and xti != 0):
        if(xti == 1 and gti != agent_id and hti > 0 and xt_gti == 2 and gt_gti == agent_id and ht_gti > 0):
            ...
            processable = max(min(backlog, int(actual_battery / e_tx_rx), remaining_framerate * proc_interval), 0)
            if(processable > 0):
            ...
            ht = min(hti, ht_gti)
            # needed_energy = ht * self._proc_interval * self.e_tx_rx
            processable = max(min(backlog, int(actual_battery / (e_tx_rx + e_frame)), remaining_framerate * proc_interval), 0)
            if(processable > 0):
                processed = min(ht * proc_interval, processable)
