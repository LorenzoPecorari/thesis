...
def jit_calculate_reward(
    ...
    processable = min(backlog, int((actual_battery - e_idle) / e_frame), processing_rate * proc_interval)    
    ...
    if(xti == 1 and gti != agent_id and hti > 0 and xt_gti == 2 and gt_gti == agent_id and ht_gti > 0):
        ...
        processable = min(backlog, int(actual_battery / e_txrx), remaining_framerate * proc_interval)
        ...
        if(actual_battery > needed_energy):
            if(processable > 0):
                ...
                offloading_reward = float(processed/processable) * (actual_battery / battery_capacity) * (processed / backlog)
        ...
        # processable = min(backlog, int(actual_battery / e_frame))
        processable = min(backlog, int(actual_battery / (e_txrx + e_frame)), remaining_framerate * proc_interval)
        processed = min(processable, ht * proc_interval)
